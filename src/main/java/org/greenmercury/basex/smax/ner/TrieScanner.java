...
public class TrieScanner {
  ...
  public class Node {
     ...
     */
    public Node put(String originalKey, String key, String val, int d) throws IllegalArgumentException {
      if (d == key.length()) {
        if (values == null)
         {
    ...
    public ArrayList<ScanResult> scan(CharSequence normalizedText, int start, int current, int end,
      ...
      if (current < end) {
        ...
        // The original text that has been matched. When there are ignored characters, this may contain more than 1 character.
        CharSequence matchedTextFragment = normalizedText.subSequence(current, nextPos);
        matchedText.append(matchedTextFragment);
        // If there were ignored characters, which included at least one whitespace character, match them as if it was one space.
        if (nextPos > current + 1 && matchedTextFragment.chars().anyMatch(c -> Character.isWhitespace(c))) {
          ch = ' ';
          nextPos = nextPos - 1;
        }
